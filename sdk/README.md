...
## Deployment model — read before deploying

> **This SDK is designed for single-process, single-instance microservices.**
> Do not run it behind a load balancer with multiple replicas, in a clustered
> process manager (PM2 `cluster` mode, Node `cluster`), or across multiple pods
> sharing the same OAuth client credentials. Doing so causes credential loss.

### Why

Several pieces of auth state live in **in-memory LRU caches** that are local to
one process and are not shared or persisted:

| State | Cache | TTL / size | Purpose |
|-------|-------|-----------|---------|
| PKCE record | `pkceStore` | 1 min, max 100 | install: verify `state` → `code_verifier` on callback |
| SID record | `sidStore` | 1 min, max 100 | install: hand the `client_token` to the frontend |
| Refresh lock | `refreshLockStore` | 5 min, max 1000 | serialize concurrent token refreshes per organization |

The **refresh lock** is the critical one. It guarantees that, within a process,
only one token refresh runs at a time per organization; concurrent callers wait
and reuse the result.

This matters because the Riseact OAuth2 provider rotates refresh tokens:

- Every successful `grant_type=refresh_token` request issues a **new** refresh
  token and revokes the old one.
- A **failed** refresh request leaves the old refresh token valid (so transient
  5xx / network errors are safe to retry).
- The old (rotated) token stays valid for a **5-minute grace period**
  (`REFRESH_TOKEN_GRACE_PERIOD_SECONDS`): reusing it within the window issues a
  fresh pair instead of failing, so a client that loses the rotation response
  (crash/restart/network drop) can recover.
- Reusing a rotated token **after** the grace window returns **HTTP 400
  `invalid_grant`** — the token is then permanently dead.

If two processes refresh concurrently with the same stored refresh token:

1. Process A rotates → gets a new token → old token revoked.
2. Process B sends the same (now-revoked) old token → `400 invalid_grant`.
3. The SDK treats a genuine `invalid_grant` as "refresh token dead" and calls
   `storage.removeCredentials(...)` → **the organization loses its credentials**
   (and, in apps that key their config off the credentials row, its config).

The in-memory lock prevents this **only within a single process**. Across
processes there is no coordination. The provider's 5-minute grace period
softens the blow — a second process reusing the old token within the window
gets a valid pair instead of `400` — but it does not fix the architecture:
the instances still churn each other's tokens, and any reuse after the window
(or two refreshes >5 min apart) still ends in `invalid_grant` and credential
loss. Run a single instance.

### If you must scale horizontally

The SDK does not support it out of the box. You would need to replace the
in-memory caches (at minimum `refreshLockStore`) with a shared store
(Redis, a database row with a lock, etc.) so the refresh lock and the
install-time PKCE/SID records are visible to every instance. Until then, run a
single instance.

## Token lifetimes (provider)

- Access token: **10 days** (`expires_in = 864000`).
- Refresh token: **infinite** lifetime, but single-use (rotated on each refresh).
- Authorization code: **30s** in production.

## Refresh behavior

- Refresh is **reactive**, not proactive: it is triggered only when a GraphQL
  request returns the error `User is not authenticated`. The SDK does not check
  `expiresDateUTC` ahead of a request.
- On refresh failure the SDK distinguishes:
  - `400` / `401` → `isRefreshTokenInvalid = true` → credentials removed
    (token genuinely dead, re-install required).
  - anything else (5xx, 429, network, malformed response, lock race) →
    credentials **kept**, error surfaced, next request retries.
