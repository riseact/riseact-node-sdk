...
export interface RiseactInstance {
  ...
  /** Development tools. It is undefined in production */
  devTools: RiseactDevTools | undefined;
  utils: RiseactTools;
  config: RiseactConfig;
}

export interface RiseactToolsMiddlewareOptions {
  publicRouter?: Router;
  protectedRouter?: Router;
  serverPort?: number;
}

export interface RiseactTools {
  /** Middleware to use in your Express app to add Riseact tools */
  startRiseactApp: (expressInstance: Express, riseact: RiseactInstance, options: RiseactToolsMiddlewareOptions) => void;
}
...
export interface CodeVerifierCookie {
  code: string;
  organizationDomain: string;
}

export interface ClientTokenCookie {
  token: string;
  organizationDomain: string;
}
...
export interface RiseactNetwork {
   ...
   * See {@link https://help.riseact.org/en/manuale/applications/ docs} to learn how to use it
   */
  gqlProxy: RequestHandler;

  /** The Apollo client to use to make GraphQL requests to Riseact from your backend. */
  createGqlClient: (organizationDomain: string, options?: ApolloClientOptions<unknown>) => Promise<ApolloClient<unknown>>;
